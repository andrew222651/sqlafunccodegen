...
class TestSQLAlchemy(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_c2vector_id(self):
        ...
        async with get_db_sesh() as db_sesh:
            result = (
                await db_sesh.execute(select(out_sqlalchemy.c2vector_id(d)))
            ).scalar_one_or_none()
        assert result is not None
    ...
    async def test_array_id(self):
        async with get_db_sesh() as db_sesh:
            result = (
                await db_sesh.execute(
                    select(out_sqlalchemy.array_id([1, 2, 3, 4]))
                )
            ).scalar_one_or_none()
        assert result is not None
        self.assertEqual(result, [1, 2, 3, 4])

    async def test_array_id_null(self):
        async with get_db_sesh() as db_sesh:
            result = (
                await db_sesh.execute(select(out_sqlalchemy.array_id(None)))
            ).scalar_one_or_none()
    ...
    async def test_jsonb_id(self):
        async with get_db_sesh() as db_sesh:
            result1 = (
                await db_sesh.execute(
                    select(out_sqlalchemy.jsonb_id([1, 2, 3, {"a": "b"}]))
                )
            ).scalar_one_or_none()
        ...
        async with get_db_sesh() as db_sesh:
            result2 = (
                await db_sesh.execute(
                    select(
                        out_sqlalchemy.jsonb_id({"key": "val"})["key"].astext
                    )
                )
            ).scalar_one_or_none()
    ...
    async def test_get_range(self):
        async with get_db_sesh() as db_sesh:
            result1 = (
                await db_sesh.execute(select(out_sqlalchemy.get_range()))
            ).scalar_one_or_none()
        assert result1 is not None
